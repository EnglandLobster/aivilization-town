...
import { createSteeringTraceApiService } from './steeringTraceApi';

describe('steering trace API service', () => {
  test('normalizes lookup and query requests before delegating', async () => {
    const calls: unknown[] = [];
    const service = createSteeringTraceApiService({
      traces: {
        getTrace: (request) => {
