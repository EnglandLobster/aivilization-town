...
describe('local runtime town profile runner CLI', () => {
  ...
  test('loads reflection synthesis config files and passes resolved config to the runner', async () => {
    let output = '';
    let receivedInput: LocalRuntimeTownProfileRunnerInput | undefined;
    const configRoot = createRootDir();
    const configPath = join(configRoot, 'profile-runtime-config.json');
        ...
        profiles: {
          'default-100': {
            reflectionSynthesis: {
              ...
              model: 'default-reflection',
              provider: {
                kind: 'openai-compatible',
                providerId: 'default-reflection-provider',
                endpoint: 'https://reflection.example.test/v1/chat/completions',
                apiKey: { env: 'AIVILIZATION_TEST_REFLECTION_LLM_KEY' },
              },
            },
          },
        },
    ...
    const previousKey = process.env.AIVILIZATION_TEST_REFLECTION_LLM_KEY;
    process.env.AIVILIZATION_TEST_REFLECTION_LLM_KEY = 'reflection-secret-key';
    ...
    try {
      ...
      expect(receivedInput?.reflectionSynthesis).toEqual({
        kind: 'traceable-llm-reflective-insight-synthesizer',
        profileId: 'default-100',
        model: 'default-reflection',
        provider: {
          ...
          providerId: 'default-reflection-provider',
          endpoint: 'https://reflection.example.test/v1/chat/completions',
          apiKey: 'reflection-secret-key',
        },
      });
    } finally {
      if (previousKey === undefined) {
        delete process.env.AIVILIZATION_TEST_REFLECTION_LLM_KEY;
      } else {
        process.env.AIVILIZATION_TEST_REFLECTION_LLM_KEY = previousKey;
      }
    }
  });

  test('loads social model synthesis config files and passes resolved config to the runner', async () => {
    let output = '';
    let receivedInput: LocalRuntimeTownProfileRunnerInput | undefined;
    const configRoot = createRootDir();
    const configPath = join(configRoot, 'profile-runtime-config.json');
        ...
        profiles: {
          'default-100': {
            socialModelSynthesis: {
              ...
              model: 'default-social-model',
              provider: {
                kind: 'openai-compatible',
                providerId: 'default-social-model-provider',
                endpoint: 'https://social-model.example.test/v1/chat/completions',
                apiKey: { env: 'AIVILIZATION_TEST_SOCIAL_MODEL_LLM_KEY' },
              },
            },
          },
        },
    ...
    const previousKey = process.env.AIVILIZATION_TEST_SOCIAL_MODEL_LLM_KEY;
    process.env.AIVILIZATION_TEST_SOCIAL_MODEL_LLM_KEY = 'social-model-secret-key';
    ...
    try {
      ...
      expect(receivedInput?.socialModelSynthesis).toEqual({
        kind: 'traceable-llm-social-model-synthesizer',
        profileId: 'default-100',
        model: 'default-social-model',
        provider: {
          ...
          providerId: 'default-social-model-provider',
          endpoint: 'https://social-model.example.test/v1/chat/completions',
          apiKey: 'social-model-secret-key',
        },
      });
    } finally {
      if (previousKey === undefined) {
        delete process.env.AIVILIZATION_TEST_SOCIAL_MODEL_LLM_KEY;
      } else {
        process.env.AIVILIZATION_TEST_SOCIAL_MODEL_LLM_KEY = previousKey;
      }
    }
  });
