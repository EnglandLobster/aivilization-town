...
function nearAnyRoad(point, edges, clearance) {
  for (const edge of edges) {
    for (let index = 1; index < edge.waypoints.length; index += 1) {
      if (segmentDistance(point, edge.waypoints[index - 1], edge.waypoints[index]) <= clearance) {
        return true;
      }
    }
  }
...
export function computeWaterRegions(locations) {
  ...
  for (const [regionId, bounds] of byRegion) {
    ...
    if (edge === 'bottom' || edge === 'top') {
      ...
      near =
        edge === 'bottom'
          ? Math.max(bounds.maxY + 0.025, occupied + 0.02)
          : Math.min(bounds.minY - 0.025, -occupied - 0.02);
      far = edge === 'bottom' ? 1 : 0;
    } else {
      ...
      near =
        edge === 'right'
          ? Math.max(bounds.maxX + 0.025, occupied + 0.02)
          : Math.min(bounds.minX - 0.025, -occupied - 0.02);
      far = edge === 'right' ? 1 : 0;
    }
