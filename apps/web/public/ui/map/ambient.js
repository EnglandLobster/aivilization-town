 ...
 */
export function computeConversationLinks(conversationRecords, nowMs, windowMs = CONVERSATION_WINDOW_MS) {
...
export function describePulseRecord(record) {
  ...
  switch (kind) {
    ...
    case 'weather-change':
      return { icon: '☂', text: record?.detail ? `Weather: ${record.detail}` : 'The weather changed' };
