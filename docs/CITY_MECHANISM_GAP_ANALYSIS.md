...
- **市民层已超越 CS**：CS 市民只是路径点；本项目 Agent 有层级规划、双过程记忆、
  ...
  库存、岗位、政策或目的地容量预留。`location-aware-action-proposer-v2` 会把上下文已知的
  满员/不可达目的地标成暂不可执行候选，交给全局综合器选择其他可行生活分支，避免 Agent
  以重复观察或重复失败的方式卡死；World 仍保留最终移动裁决权。
...
> **2026-08-31 P8 无房反馈切片：`town-wellbeing-v2` 不再把住宅品质档位误当成实际住所；
> 无 durable residence 的 Agent 承受独立幸福惩罚，继而进入既有幸福→迁出反馈。自主生活路线
> `autonomous-life-course-v4` 会在存在权威空位时优先形成找房目标，移动到住宅后仍由 authority
> 串行领取空位。v1 policy 和未启用居所分配的运行不注入该因子，历史事件继续按最终事实回放。**
> **2026-08-31 P8 自主供给切片：启用 `town-construction-v1` 后，规划上下文会用 society
> 决策函数与版本化规划规则投影全镇住房压力、材料缺口、目标地点及分区内确定性建造者；合资格 Agent 主动形成
> 扩建目标，不再依赖偶然出现的住宅升级任务“搭车”进入建造命令。最终材料扣减与容量增加仍由
> simulation-wide authority 原子结算；没有新增功能开关。**
> 以下为原始排序，保留作历史脉络：
