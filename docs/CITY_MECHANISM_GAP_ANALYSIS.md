...
- **城市层仍未达到 CS**：全镇仍是固定地点抽象图（`packages/content/src/locations.ts`），
  没有建造、区划、实体物流与需求驱动迁入；但区域地价、财政、生命周期、服务质量和
...
- 现状：世界昼夜相位与生理被动衰减已由 `town-calendar` 结算；7 种天气 Markov 链
  （`packages/world/src/weather.ts`）已接入 soaked/cold 与前端图层。初级资源仍凭空产出
  （`inputs: {}`），没有储量。
- 仍缺：营业时段联动、季节对生产的影响、资源节点与枯竭、污染和偶发灾害。
...
剩余缺口：A 空间/建造、B 交通物流、出生/家庭、人口迁入及更完整治理制度。
E 的税率/预算/补贴已经是参与者和合资格 Agent 可调的权威命令。企业加入/注资、银行贷款与
企业侧外贸已有 deterministic proposer；企业创办及更完整的 LLM 自主营运仍待补齐。
...
- **幸福驱动迁出**（`--town-migration`，默认关）：CS2 NotHappy 多项式形状
  ...
  互锁显式。迁入（需求驱动）为后续扩展（届时提升 policy 版本）。
...
> P5 服务短缺与 P6 治理命令面均已落地（新机制 flag 默认关）。下一项 P7 需求与人口流动。**
> 以下为原始排序，保留作历史脉络：
