...
剩余缺口：A 空间/建造、B 交通物流、C 服务短缺动力学、D 人口生命周期、
E 的治理命令面（税率/预算仍是参数而非参与者可调命令）。企业加入/注资、银行贷款与
...
- **请愿集体行动**（`--town-collective-action`，默认关）：`society/collectiveAction.ts`
  ...
  预留给 P6 治理命令面做输入）；未达阈值的公开请愿 3 模拟日后过期。
...
> **2026-08-15 进度（按 ROI 重排）：P1–P3、P4a/P4c、P4b-1（请愿）、S5 迁出
> 已落地（flag 默认关）。下一项 P5 服务短缺 → P6 治理命令面。**
> 以下为原始排序，保留作历史脉络：
