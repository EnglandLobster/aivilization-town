...
- `public/ui/assets/tiles.png` is a 16px-tile pixel sheet generated by
  `apps/web/tools/generate-tilesheet.mjs` (dependency-free Node: hand-rolled PNG encoder over
  `node:zlib`). Building sprites are organized as 7 location kinds × 3 upgrade levels; only level 1
  is drawn today — levels 2–3 are reserved frames for spatial growth (#8), and a day/night tint
  layer is reserved for the world clock (#12). Re-run the generator after changing it; the PNG is
  committed.
