...
authority and materializes authoritative inboxes before each tick. Remaining
migration gates before any multi-partition market, town or scale result can be
described as a unified society: merging the per-partition AMM pools into one
global pool and retiring the legacy AMM as the default path; routing cross-owner
movement through the authority once the owner-transfer runtime handoff (Agent
storage migration and replay materialization) exists; and enabling the
authority as the default run path rather than an opt-in. The current town
projection remains an honest read model for the local host: with the authority
disabled (the default) its AMM pools are still separate, and with it enabled the
unified pool has not yet merged multi-partition market state.
